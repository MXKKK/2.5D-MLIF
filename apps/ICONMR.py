...
from lib.net import HGPIFuMRNet
...
class ICONMR(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        self.ignore = ['C_normal_F', 'C_normal_B']

        self.netG = HGPIFuMRNet(
            self.cfg,
        ...
        self.coarse_reconEngine = Seg3dLossless(
        ...
        self.in_nml = [item[0] for item in cfg.net.in_nml_local]
        self.in_geo_dim = [item[1] for item in cfg.net.in_geo_local]
        self.show_geo_dim = [item[1] for item in cfg.net.in_geo_local]
    ...
    def configure_optimizers(self):
        ...
        optim_params_G.append({"params": self.netG.B_filter.parameters(), "lr": self.lr_G})
    ...
    def training_step(self, batch, batch_idx):
        ...
        in_tensor_dict = {
            ...
            "sample_crop":batch["pts_crop"].permute(0, 2, 1),
            "calib": batch["calib"],
        ...
        for name in self.in_geo:
            if name not in self.ignore:
                in_tensor_dict.update({name + '_512': batch[name + '_512']})
                in_tensor_dict.update({name + '_crop': batch[name + '_crop']})
    ...
    def test_step(self, batch, batch_idx):
        ...
        #image_inter = np.concatenate(self.tensor2image(1024, inter[0]) +
                                     #[smpl_F, smpl_B, image],
                                     #axis=1)
        image_inter = np.concatenate(self.tensor2image(1024, inter[0]) +
                                     [image],
    ...
    def render_func(self, in_tensor_dict, dataset="title", idx=0):
        ...
        self.netG.eval()
        features, inter, coarse_features = self.netG.filter(in_tensor_dict, return_inter=True)
        sdf = self.reconEngine(opt=self.cfg,
                               netG=self.netG,
                               features=features,
    ...
    def test_single(self, batch, repair=False):
        ...
        for name in self.in_total:
            if name in batch.keys():
                in_tensor_dict.update({name: batch[name]})
            name_512 = name + '_512'
            if name_512 in batch.keys():
                in_tensor_dict.update({name_512: batch[name_512]})
            name_crop = name + '_crop'
            if name_crop in batch.keys():
                in_tensor_dict.update({name_crop: batch[name_crop]})
