...
from lib.dataset.TestMRDataset import TestMRDataset
from lib.net.local_affine import LocalAffine
from pytorch3d.structures import Meshes
from apps.ICONMR import ICONMR
from apps.Normal import Normal
from apps.Depth import Depth
...
def test_sample_coordinate(data):
    ...
    '''
    crop_windows_size = 512
    img = data['image']
    front_normal = data['F_normal_F']
    back_normal = data['F_normal_B']
    front_depth = data['F_depth_F']
    back_depth = data['F_depth_B']



    #data['image'] = F.interpolate(img,(crop_windows_size,crop_windows_size),mode='bilinear', align_corners=True)
    data['F_normal_F'] = F.interpolate(front_normal,(crop_windows_size,crop_windows_size),mode='bilinear', align_corners=True)
    data['F_normal_B'] = F.interpolate(back_normal,(crop_windows_size,crop_windows_size),mode='bilinear', align_corners=True)
    data['F_depth_F']  = F.interpolate(front_depth,(crop_windows_size,crop_windows_size),mode='bilinear', align_corners=True)
    data['F_depth_B']  = F.interpolate(back_depth,(crop_windows_size,crop_windows_size),mode='bilinear', align_corners=True)


    data['image_crop'] = img
    data['F_normal_F_crop'] = front_normal
    data['F_normal_B_crop'] = back_normal
    data['F_depth_F_crop']  = front_depth
    data['F_depth_B_crop']  =  back_depth
...
if __name__ == "__main__":
    ...
    # load normal-net
    # cfg read and merge
    cfg.merge_from_file(args.norm_cfg)
    normal_model = Normal(cfg)
    pretrained_dict = torch.load(
            cfg.resume_path,
            map_location=torch.device(f"cuda:{0}"))["state_dict"]
    model_dict = normal_model.state_dict()
    ...
    normal_model.load_state_dict(model_dict)
    normal_model.to(device)
    del pretrained_dict
    del model_dict

     # load depth-net
    cfg.merge_from_file(args.dept_cfg)
    depth_model = Depth(cfg)
    ...
    model_dict.update(pretrained_dict)
    # 3. load the new state dict
    depth_model.load_state_dict(model_dict)
    ...
    dataset = TestMRDataset(dataset_param, device)
    ...
    for data in pbar:
        ...
        # tmsk = torch.repeat_interleave(data["mask"], 3, dim=0)
        
        # print(args.in_dir + data['name'] + '_mask.png')
        # msk = Image.fromarray(np.uint8(np.transpose(tmsk.cpu().numpy(), (1,2,0)) * 255.0))
        # msk.save(args.in_dir + data['name'] + '_mask.png')
        # img = Image.fromarray(np.uint8((np.transpose(data["image"][0].cpu().numpy(), (1,2,0) ) + 1.0 )/ 2 * 255.0))
        # img.save(args.in_dir + data['name'] + '_img.png')
        ...
        for i in loop_smpl:
            ...
            with torch.no_grad():
                in_tensor["normal_F"], in_tensor[
                    "normal_B"] = normal_model.test_single(in_tensor)
            ...
            if i % args.vis_freq == 0:
                ...
                per_loop_lst.extend([
                    ...
                    in_tensor["normal_F"],
                    diff_F_smpl / 2.0,
                    diff_S[:, :1024].unsqueeze(0).unsqueeze(0).repeat(
                        1, 3, 1, 1),
                ])
                per_loop_lst.extend([
                    in_tensor["image"],
                    ...
                    diff_B_smpl / 2.0,
                    diff_S[:,
                           1024:].unsqueeze(0).unsqueeze(0).repeat(1, 3, 1, 1),
                ])
                per_data_lst.append(
        ...
        down_sample = transforms.Resize(512)
        in_tensor["F_normal_F_512"] = down_sample(in_tensor["normal_F"])
        in_tensor["F_normal_B_512"] = down_sample(in_tensor["normal_B"])
            ...
            for i in loop_cloth:
                ...
                with torch.no_grad():
                    if i % args.vis_freq == 0:
                        ...
                        # print(in_tensor["image"].shape)
                        # print(in_tensor["P_normal_F"].shape)
                        # print(in_tensor["normal_F"].shape)
                        per_loop_lst.extend([
                            in_tensor["image"],
