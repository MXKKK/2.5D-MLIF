...
from lib.net.HGFiltersHD import *
...
class HGPIFuMRNet(BasePIFuNet):
    ...
    def __init__(self,
        ...
        # TODO: add to config
        self.coarse_feature_dim = 256
        #self.mlp_dim = [256, 512, 256, 128, 1]

        self.mlp_dim = [512, 1024, 512, 256, 1]

        # self.mlp_dim = [512, 256, 128, 1]
    ...
    def get_normal(self, in_tensor_dict):
        ...
        in_filter = torch.cat([in_tensor_dict[key+'_crop'] for key in self.in_geo],
                                dim=1)
        # in_filter_global = torch.cat([in_tensor_dict[key] for key in self.in_geo],
        #                         dim=1)

        return in_filter
    
    def get_normal_global(self, in_tensor_dict):
        ...
        return in_filter,in_filter_global
    ...
    def filter(self, in_tensor_dict, return_inter=False):
        ...
        if self.prior_type in ["icon", "keypoint"]:
            if self.use_filter:
                ...
                # features_Gl = self.G_filter(in_filter[:,
                #                                      self.channels_filter[1]]
                #                            )  # [(B,hg_dim,128,128) * 4]
            else: 
            ...
            # for idx in range(len(features_Gl)):
            #     features_G.append(
            #         torch.cat([features_F[0], features_B[0], features_Gl[idx]], dim=1))
    ...
    def query(self, features, global_features, points, points_crop, calibs, transforms=None, regressor=None):
        if self.train_together:
    ...
    def forward(self, in_tensor_dict):
        ...
        sample_crop_tensor = in_tensor_dict["sample_crop"]
