...
class DepthNet(BasePIFuNet):
    ...
    def __init__(self, cfg, error_term=nn.SmoothL1Loss()):
        ...
        super(DepthNet, self).__init__(error_term=error_term)
        ...
        self.netF = define_G(self.in_nmlF_dim, 1, 64, "global", 4, 9, 1, 3,
                             "instance")
        self.netB = define_G(self.in_nmlB_dim, 1, 64, "global", 4, 9, 1, 3,
                             "instance")
    ...
    def get_norm_error(self, prd_F, prd_B, tgt, use_vgg=True):
        """calculate normal loss

        Args:
            pred (torch.tensor): [B, 2, 512, 512]
            tagt (torch.tensor): [B, 2, 512, 512]
        """

        tgt_F, tgt_B = tgt['depth_F'], tgt['depth_B']
        ...
        if(not use_vgg):
            total_loss = [
                100.0 * l1_F_loss, 100.0 * l1_B_loss
            ]
        else:
            with torch.no_grad():
                vgg_F_loss = self.vgg_loss[0](torch.repeat_interleave(prd_F, 3, dim=1), torch.repeat_interleave(tgt_F, 3, dim=1))
                vgg_B_loss = self.vgg_loss[0](torch.repeat_interleave(prd_B, 3, dim=1), torch.repeat_interleave(tgt_B, 3, dim=1))
