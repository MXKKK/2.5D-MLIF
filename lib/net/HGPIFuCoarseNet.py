...
class HGPIFuCoarseNet(BasePIFuNet):
    """
    ...
    def __init__(self,
        ...
        super(HGPIFuCoarseNet, self).__init__(projection_mode=projection_mode,
        ...
        self.ignore = ['C_normal_F', 'C_normal_B', 'depth_F', 'depth_B']

        t_in_geo = []
        for item in self.opt.in_geo:
        ...
        self.in_geo_dim = sum([item[1] for item in t_in_geo])
    ...
    def get_normal(self, in_tensor_dict):
        ...
        in_filter = torch.cat([in_tensor_dict[key + '_512'] for key in self.in_geo],
                                dim=1)
    ...
    def query(self, features, points, calibs, transforms=None, regressor=None, update_phi=False):
        ...
        for im_feat, vol_feat in zip(features, vol_feats):
            ...
            if update_phi:
                preds, phi = regressor(point_feat, returnPhi=update_phi)
            else:
                preds = regressor(point_feat, returnPhi=update_phi)
            if update_phi:
                self.phi = phi
            preds = in_cube * preds
    ...
    def forward(self, in_tensor_dict, return_phi=False):
        """
        sample_tensor [B, 3, N]
        calib_tensor [B, 4, 4]
        label_tensor [B, 1, N]
