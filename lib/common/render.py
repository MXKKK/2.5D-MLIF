...
class Render:
    ...
    def __init__(self, size=512, dis=100,device=torch.device("cuda:0")):
        self.device = device
        self.size = size
    ...
    def reload_cam(self):
        ...
        self.cam_pos = [
            (0, self.mesh_y_center, self.dis - 0.4),
    ...
    def get_depth_map(self, cam_ids=[0, 2]):
        ...
        for cam_id in cam_ids:
            ...
            zbuf =  fragments.zbuf
            zbuf[fragments.pix_to_face < 0] = 101.0
            t = zbuf[..., 0][fragments.pix_to_face[..., 0] >= 0].clone()
            t = (t - 99.0) / 2.0
            means.append(t.mean())
            stds.append(t.std())
            depth_map = zbuf[..., 0].squeeze(0)
            if cam_id == 2:
                depth_map = torch.fliplr(depth_map)
            depth_maps.append(depth_map)

        return depth_maps, means, stds
