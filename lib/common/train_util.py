...
def load_networksMR(cfg, model, mlp_path, coarse_path):
    model_dict = model.state_dict()
    main_dict = {}
    normal_dict = {}
    ...
    # normal network part loading
    if os.path.exists(coarse_path) and coarse_path.endswith("ckpt"):
        normal_dict = torch.load(
            coarse_path,
            map_location=torch.device(f"cuda:{cfg.gpus[0]}"))["state_dict"]
        ...
        print(colored(f"Resume normal model from {coarse_path}", "green"))
...
def query_func_MR(opt, netG, features, coarse_features, points, proj_matrix=None):
    '''
        - points: size of (bz, N, 3)
        - proj_matrix: size of (bz, 4, 4)
    return: size of (bz, 1, N)
    ...
    preds = netG.query(features=features,
                       global_features=coarse_features,
                       points=samples,
                       calibs=calib_tensor,
                       regressor=netG.if_regressor)
...
def query_func_MR2(opt, netG, features, coarse_features, points,proj_matrix=None):
    '''
        - points: size of (bz, N, 3)
        - proj_matrix: size of (bz, 4, 4)
    return: size of (bz, 1, N)
    ...
    if type(preds) is list:
        preds = preds[0]
    return preds        
def query_func_coarse(netG, features, samples, calib_tensor):
    '''
        - netG: the actual query network (i.e. HGPIFuNet)
        - samples: size of (bz, 3, N)
    return:
        - preds: size of (bz, 1, N)
        - phi:   size of (bz, phi_channel, N)
    '''
    preds = netG.query(features=features,
                       points=samples,
                       calibs=calib_tensor,
...
def query_func_refine(opt, netG, features, features_coarse, points, proj_matrix=None):
    '''
        - points: size of (bz, N, 3)
        - proj_matrix: size of (bz, 4, 4)
    return: size of (bz, 1, N)
    ...
    preds, phi = query_func_coarse(netG=netG.netG.netG,
