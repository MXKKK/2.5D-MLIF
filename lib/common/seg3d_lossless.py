...
class Seg3dLossless(nn.Module):
    ...
    def get_FB_normal(self, sdf):

        # render
        X, Y, Z, norm = self.find_vertices(sdf, direction="front")
        image1 = self.render_normal(512, X, Y, Z, norm)
        image1 = image1.detach().cpu().numpy()[0].transpose(1, 2, 0)
        

        X, Y, Z, norm = self.find_vertices(sdf, direction="back")
        image2 = self.render_normal(512, X, Y, Z, norm)
        image2 = image2.detach().cpu().numpy()[0].transpose(1, 2, 0)
