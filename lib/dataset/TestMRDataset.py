...
class TestMRDataset():
    ...
    def __init__(self, cfg, device):
        ...
        self.render = Render(size=1024, device=device)
    ...
    def __getitem__(self, index):
        ...
        if self.seg_dir is None:
            img_icon, img_hps, img_ori, img_mask, uncrop_param = process_image(
                img_path, self.hps_type, 1024, self.device)
            ...
            data_dict = {
                ...
                'ori_image': img_ori,
                'image_512': down_sample(img_icon).to(self.device).unsqueeze(0),
                'mask': img_mask,
                'mask_512': down_sample(img_mask),
        ...
        # summary(self.hps, img_hps.shape[1:])
        with torch.no_grad():
